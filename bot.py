...
def get_all_records() -> list[dict]:
    """Retorna todos os registros da aba atual como lista de dicts."""
    ws = get_sheet()
    rows = ws.get_all_values()
    if len(rows) < 8:
        return []
    # linha 7 (índice 6) é o cabeçalho na sua planilha
    headers = rows[6]
    records = []
    for row in rows[7:]:
...
def build_financial_summary() -> str:
    """Monta um resumo financeiro em texto para passar ao Claude."""
    ...
    resumo = (
        ...
        f"--- Todos os lançamentos ---\n" + "\n".join(lancamentos[-60:])  # últimos 60
    )
    return resumo
...
def is_financial_question(text: str) -> bool:
    """Detecta se o texto é uma pergunta ao consultor ou um lançamento."""
    question_keywords = [
    ...
    # Se tem verbo de lançamento, provavelmente é lançamento
...
def consult_claude(question: str, financial_summary: str) -> str:
    prompt = (
...
def generate_alert(data: dict, financial_summary: str) -> str | None:
    """Gera alerta inteligente após registrar lançamento, se necessário."""
    prompt = (
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Consulta financeira ou lançamento ──
...
async def handle_lancamento(update: Update, text: str):
    ...
    try:
        ...
        # Gera alerta inteligente em background
        try:
...
async def handle_consulta(update: Update, question: str):
    ...
    try:
        summary = build_financial_summary()
        answer = consult_claude(question, summary)
